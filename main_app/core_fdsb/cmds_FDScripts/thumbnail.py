...
def resolve_inline(args: list[str], ctx: ExecutionContext) -> str:
    if args:
        ...
        if url:
            ctx.get_embed_builder(index).thumbnail = url
    return ""
...
async def execute(cmd: Command, args: list[str], ctx: ExecutionContext, ch: discord.abc.Messageable) -> None:
    if not args:
        await _send_error(ch, FDLogicError(
            "`$thumbnail` requires at least one argument: $thumbnail[url;index (optional)]"
        ))
        return
    ...
    if not url:
        await _send_error(ch, FDLogicError(
            "`$thumbnail` — no valid URL found in the given argument(s). "
            "The resolved text must contain a URL starting with http:// or https://"
        ))
        return

    ctx.get_embed_builder(index).thumbnail = url
    ctx.log_event(f"thumbnail → {url!r} (index {index})")