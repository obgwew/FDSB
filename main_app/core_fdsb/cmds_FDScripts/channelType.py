...
def resolve_inline(args: list[str], ctx: ExecutionContext) -> str | None:
    ...
    if not args or not args[0].strip():
        channel = ctx.interaction.channel if ctx.interaction else ctx.message.channel
    else:
    ...
    return str(channel.type.name)

async def execute(cmd: Command, args: list[str], ctx: ExecutionContext, ch: discord.abc.Messageable) -> None:
    value = resolve_inline(args, ctx)
    if value is None:
        value = "unknown"
    ...
    ctx.log_event(f"channelType{arg_log} → {value!r}")