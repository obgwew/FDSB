...
async def _process(args: list[str], ctx: ExecutionContext) -> str:
    if len(args) < 1:
        ctx._abort_with_error(FDSyntaxError("`$httpDelete` requires at least 1 arg: `$httpDelete[URL;(Body)]`"))
    ...
    try:
        ...
        async with session.delete(url, headers=headers, json=json_data, data=data) as res:
