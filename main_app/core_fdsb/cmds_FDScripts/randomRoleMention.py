# cmds_FDScripts/randomRoleMention.py
import random
import discord
...
def resolve_inline(args: list[str], ctx: ExecutionContext) -> str:
    ...
    if not roles:
        return ""
    return random.choice(roles).mention
...
async def execute(cmd: Command, args: list[str], ctx: ExecutionContext, ch: discord.abc.Messageable) -> None:
    ...
    if guild is None:
        await _send_error(ch, FDLogicError(
            "`$randomRoleMention` — this command can only be used inside a server (not in DMs)"
        ))
        return
    ...
    if not roles:
        await _send_error(ch, FDEnvironmentError(
            "`$randomRoleMention` — this server has no roles to pick from"
        ))
        return
    ...
    ctx.log_event(f"randomRoleMention → {result} ({role.name})")