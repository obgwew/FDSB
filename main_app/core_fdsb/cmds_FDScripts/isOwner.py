...
def resolve_inline(args: list[str], ctx: ExecutionContext) -> str:
    ...
    return "true" if guild.owner_id == user_id else "false"


async def execute(cmd: Command, args: list[str], ctx: ExecutionContext, ch: discord.abc.Messageable) -> None:
    if not args:
        await _send_error(ch, FDLogicError(
            "`$isOwner` requires a user ID or mention argument: `$isOwner[userID]`"
        ))
        return
    ...
    ctx.log_event(f"isOwner → {result} for user `{ctx.resolve(args[0]).strip()}`")